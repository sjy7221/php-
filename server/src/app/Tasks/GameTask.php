...
class GameTask extends Task
{
    ...
    public function kaipai()
    {
    	...
    	$time = time();
    	//房间号有问题
    	 $room  =  yield $this->redis_pool->getCoroutine()->hgetall($this->room_id);//取出房间信息及押注信息
    	 $user  =  yield $this->redis_pool->getCoroutine()->hgetall('uids_' . $this->room_id);//取出所有用户信息
    	 $uids = yield $this->redis_pool->getCoroutine()->hkeys('uids_' . $this->room_id);//取出所有人的id
    	 $roomInfo =  unserialize($room['roomInfo']);
    	 $userInfo = 	unserialize($room['userInfo']);
    	 $pais = $roomInfo['pai'];//房间剩余牌 //////////分控操作pais
    	$jieguo =  getPai($pais);
    	if(!$jieguo){
    		$pai = washCards();
    		$roomInfo['pai'] = $pai;
    	...
    	// yield $this->redis_pool->getCoroutine()->hset($this->room_id,'roomInfo',serialize($user));
    	...
    	for($i=0;$i<100;$i++){
				...
				//[mid-z-金额]
				$zhu = '0-'.$zxh.'-'.$money;
				 yield $this->redis_pool->getCoroutine()->lpush('zonghe_'.$this->room_id,$zhu);//压入总注池子
				 yield $this->redis_pool->getCoroutine()->hset($this->room_id,'roomInfo',serialize($user));
			$data = [
            'mid'=>0,
            'roomInfo'=>$roomInfo,
    	...
    	foreach ($userInfo as $k => $v) { //！！！！！！！！！！！！
    		...
    		$user['numb'] += $zongwin;
    		$user['numb'] -= $zongshu;
    		yield $this->redis_pool->getCoroutine()->hset('uids_' . $this->room_id,$k,serialize($user));
    		$data = [
    			'win'=>$zongwin,
    			'shu'=>$zongshu,
    			'user'=>$user
    	...
    	$roomInfo['zj']['numb'] += $ying; //庄家用户输赢分数
    	$roomInfo['luzi'][] = $one;
    	yield $this->redis_pool->getCoroutine()->hset($this->room_id,'roomInfo',serialize($user));
    	  $this->destroy();
    }
